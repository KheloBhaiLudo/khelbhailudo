...
// ========================================================
// 💰 CASHFREE PAYMENT SUCCESS & WALLET UPDATE LOGIC
// ========================================================

// 1. Frontend verification route (Jab return_url hit hoga)
app.post('/api/payment/verify-status', async (req, res) => {
    try {
        ...
        // Cashfree se order ka status check karne ke liye API call
        ...
        // Agar order success hai (PAID status)
        if (orderInfo.order_status === "PAID") {
            const amount = parseFloat(orderInfo.order_amount);
            
            // order_id humne banaya tha: ORD_timestamp_userId
            const userId = orderInfo.order_id.split('_')[2]; 

            // --- DATABASE UPDATE LAYER ---
            // ⚠️ Praveen, yahan tumhare Supabase table ka query structure aayega
            // Maan lete hain tumhare table ka naam 'users' aur column 'wallet_balance' hai:
            
            // Pehle current balance nikalte hain
            ...
            return res.json({ success: true, message: "Wallet successfully updated", amount: amount });
        } else {
            return res.json({ success: false, message: `Order status is ${orderInfo.order_status}` });
        }
